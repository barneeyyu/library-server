...
3. **新增書籍副本到圖書館** - 測試為現有書籍新增副本
4. **新增重複書籍（應該失敗）** - 測試重複書籍新增的錯誤處理
5. **搜尋書籍（依書名）** - 測試書名搜尋功能
6. **搜尋書籍（依作者）** - 測試作者搜尋功能
7. **搜尋書籍（組合條件）** - 測試多條件組合搜尋
8. **搜尋書籍（依圖書館）** - 測試依圖書館ID搜尋書籍
9. **搜尋書籍（無條件 - 應該失敗）** - 測試搜尋驗證
10. **取得書籍詳情** - 測試書籍詳情查詢
11. **取得不存在書籍** - 測試 404 錯誤處理

### Borrow - 借閱與還書
1. **會員借書（成功）** - 測試會員成功借閱書籍
2. **借書（已借閱同一本書）** - 測試重複借閱同一本書的錯誤處理
3. **借書（書籍副本不存在）** - 測試借閱不存在副本的錯誤處理
4. **查詢個人借閱記錄** - 測試會員查詢自己的借閱歷史
5. **查詢目前借閱中的書籍** - 測試會員查詢目前未歸還的書籍
6. **查詢借閱限制信息** - 測試會員查詢不同書籍類型的借閱限制
7. **會員還書（成功）** - 測試會員成功歸還書籍
8. **還書（借閱記錄不存在）** - 測試歸還不存在借閱記錄的錯誤處理
9. **重複還書（應該失敗）** - 測試重複歸還已歸還書籍的錯誤處理
10. **館員查詢逾期書籍** - 測試館員查詢所有逾期未歸還的書籍
11. **會員查詢逾期書籍（應該失敗）** - 測試會員無權限查詢逾期書籍
12. **館員發送到期通知** - 測試館員發送書籍到期歸還提醒
13. **會員發送到期通知（應該失敗）** - 測試會員無權限發送通知
...
| `test_book_copy_id` | 測試書籍副本 ID | 自動設定 | Collection |
| `test_borrow_record_id` | 測試借閱記錄 ID | 自動設定 | Collection |
...
- ❌ 新增重複書籍失敗 (400 Bad Request)
- ❌ 無搜尋條件的查詢被拒絕 (400 Bad Request)
- ❌ 不存在書籍返回 404 (404 Not Found)
- ❌ 無 token 訪問受保護端點失敗 (401 Unauthorized)
- ❌ 重複借閱同一本書失敗 (400 Bad Request)
- ❌ 借閱不存在書籍副本失敗 (400 Bad Request)
- ❌ 還書不存在的借閱記錄失敗 (404 Not Found)
- ❌ 重複還書失敗 (400 Bad Request)
- ❌ 會員無權查詢逾期書籍失敗 (403 Forbidden)
- ❌ 會員無權發送到期通知失敗 (403 Forbidden)
...
- 書籍副本 ID 自動提取和儲存 (Collection 變數)
- 借閱記錄 ID 自動提取和儲存 (Collection 變數)
- 權限控制驗證
- 搜尋結果驗證
- 借閱狀態驗證
- 還書狀態驗證
